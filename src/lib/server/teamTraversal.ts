...
export const verifyOwnership = async (user: UserInterface, team: TeamInterface): Promise<void> => {
    if (!user._id.equals(team.owner)) throw {
        status: 401,
        message: "You are not the owner of this team"
    }
}

type TeamTraversalFunction<Input, Output> = (user: UserInterface | null |  undefined, body: Input, mustBeOwner?: boolean) => Promise<Output>;

////////// TEAM

type TeamInput = { teamId: string }
type TeamOutput = { user: UserInterface, team: TeamInterface}
export const getTeamAndUser: TeamTraversalFunction<TeamInput, TeamOutput> = async (user, body, mustBeOwner = true) => {
    ...
    if (mustBeOwner) await verifyOwnership(user, team);
    return { team, user }
}
...
type ProjectOutput = { team: TeamInterface, project: ProjectInterface, projectIndex: number }
export const traverseTeamForProject: TeamTraversalFunction<ProjectInput, ProjectOutput> = async (user, body, mustBeOwner = true) => {
    const { team } = await getTeamAndUser(user, body, mustBeOwner)
    ...
    return { team, projectIndex, project: team.projects[projectIndex] }
}
...
export const traverseTeamForTrack: TeamTraversalFunction<TrackInput, TrackOutput> = async (user, body, mustBeOwner = true) => {
    const data = await traverseTeamForProject(user, body, mustBeOwner)
...
export const traverseTeamForDemo: TeamTraversalFunction<DemoInput, DemoOutput> = async (user, body, mustBeOwner = true) => {
    const data = await traverseTeamForTrack(user, body, mustBeOwner)
...
export const traverseTeamForComment: TeamTraversalFunction<CommentInput, CommentOutput> = async (user, body, mustBeOwner = true) => {
    const data = await traverseTeamForDemo(user, body, mustBeOwner)
...
export const traverseTeamForInspiration: TeamTraversalFunction<InspirationInput, InspirationOutput> = async (user, body, mustBeOwner = true) => {
    const data = await traverseTeamForDemo(user, body, mustBeOwner)
